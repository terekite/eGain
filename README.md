...
- Navigate to the eGain/src/ directory in your terminal.
...
come up throughout the tool. A Prompt is an object which contains a message and input validations, displays/prompts
...
to another prompt. The third and final step was to start the loop by creating a PromptChain object and calling run() on it,
and providing it with a starting prompt. The PromptChain is essentially the chatbot, which loops until the next prompt is null.
